...
//  AI（実カード対応）
// ============================================================
test('AI：ボスの指令で弱ったベンチを引きずり出す', () => {
  registerLocalCards({ 'ai-boss': { id: 'ai-boss', name: 'ボスの指令', category: 'Trainer', trainerType: 'Supporter', effect: { kind: 'unimplemented' } } });
  const g = new Game(DECKS.fire, DECKS.water, { rng: rngFrom(41) });
  g.players[0].setupDone = true; g.players[1].setupDone = true;
  g.turnPlayer = 0; g.turnCount = 2; g.phase = 'main';
  const atk = new PokemonInPlay('fluffit'); atk.energy = ['energy-fire', 'energy-fire']; g.players[0].active = atk;
  g.players[0].hand = ['ai-boss'];
  g.players[1].active = new PokemonInPlay('aqualet');
  const weak = new PokemonInPlay('sprout'); weak.damage = 50; g.players[1].bench = [weak]; // 残り10
  const ai = new AI(g, 0);
  ai._useSupporter();
  eq(g.players[1].active.cardId, 'sprout', '弱ったベンチを引きずり出す');
  assert(g.players[0].discard.includes('ai-boss'), 'ボスの指令を消費');
});
...
// ============================================================
//  特殊エネルギーの「つけたとき」効果（AI自動解決）
// ============================================================
test('リッチエネルギー：つけたとき4枚引く', () => {
  registerLocalCards({ 'se-rich': { id: 'se-rich', name: 'リッチエネルギー', category: 'Energy', energyType: 'Colorless', special: true } });
  const g = new Game(DECKS.fire, DECKS.water, { rng: rngFrom(91) });
  g.turnPlayer = 0; g.phase = 'main';
  const a = new PokemonInPlay('embor'); g.players[0].active = a;
  g.players[0].hand = ['se-rich']; g.players[0].deck = Array(6).fill('energy-fire');
  new AI(g, 0)._attach(0, a.uid);
  eq(a.energy.length, 1, 'エネがついた');
  eq(g.players[0].hand.length, 4, '4枚引いた（手札1→消費0→+4）');
});

test('テレパス超エネルギー：つけたとき超のたねを2枚ベンチに', () => {
  registerLocalCards({ 'se-tele': { id: 'se-tele', name: 'テレパス超エネルギー', category: 'Energy', energyType: 'Psychic', special: true } });
  const g = new Game(DECKS.fire, DECKS.water, { rng: rngFrom(92) });
  g.turnPlayer = 0; g.phase = 'main';
  const a = new PokemonInPlay('mindle'); g.players[0].active = a; g.players[0].bench = [];
  g.players[0].hand = ['se-tele']; g.players[0].deck = ['mindle', 'mindle', 'energy-fire']; // mindle=超たね
  new AI(g, 0)._attach(0, a.uid);
  eq(a.energy.length, 1, 'エネがついた');
  eq(g.players[0].bench.length, 2, '超のたね2枚をベンチに');
});
